...
const App = () => {
    ...
    useEffect(() => {
        ...
        const finalizeRonde = (scores, wasErOnderDoor = false) => {
            ...
            const nieuweTotalen = berekenTotalen(checkRondes);
        ...
        return (
            ...
            <div className="w-full max-md bg-slate-900 shadow-2xl flex flex-col relative overflow-hidden h-screen border-x border-slate-800">
            ...
            {actieveTab === 'SCOREBORD' && (
                ...
                {rondes.length === 0 ? (
                    ...
                    rondes.map((ronde, rIndex) => (
                        ...
                        <button onClick={() => startWijzigenRonde(rIndex)} className="text-slate-600 hover:text-emerald-400 p-1 mr-2"><Edit2 className="w-4 h-4" /></button>
                        </td>
                        </tr>
                    ))
                )}
            ...
            {actieveTab === 'INVOEREN' && (
                ...
                <button onClick={probeerRondeToeTeVoegen} className="w-full bg-emerald-500 text-slate-900 font-extrabold py-4 rounded-xl shadow-lg mt-6 flex items-center justify-center transition-all"><Check className="w-5 h-5 mr-2" /> {editRondeIndex !== null ? "Wijziging Opslaan" : "Ronde Opslaan"}</button>

                {editRondeIndex !== null && (
                    <button onClick={() => { setEditRondeIndex(null); setHuidigeRondeInput(Array(aantalSpelers).fill(0)); setActieveTab('SCOREBORD'); }} className="w-full bg-slate-800 text-slate-400 py-3 rounded-xl mt-2 font-bold">Annuleren</button>
                )}
                </div>
            )}
            ...
            {popupSpelerIndex !== null && (
                ...
                <button onClick={() => setPopupSpelerIndex(null)} className="bg-slate-800 p-2 rounded-full text-slate-400"><X className="w-6 h-6" /></button>
                </div>
